    ...
    <div class="main-bg">
      <div class="main-inside">
        <div class="assets-common-container">
          <h3 style="font-size:24px;">资产归还流程</h3>
          <el-row>
            <el-col :span="24" style="padding-right:10px">
              <el-row class="col-height-40">
                ...
                <el-col class="apply-title-content" :span="20">
                  <span
                    style="float:right"
                  >{{state?draftData.title:'资产归还-'+userInfo.trueName}}-{{formatTitle}}</span>
                </el-col>
              </el-row>
              <el-row class="col-height-40">
...
export default {
  ...
  methods: {
    postSave(data) {
      this.$http.post("assetReturn/save", data).then(res => {
        this.$notify({
          title: "成功",
    ...
    assetReturnAdd(val) {
      if (this.tableData.length > 0) {
        ...
        if (!this.state) {
            ...
            .then(
              res => {
                ...
                if (val == 2) {
                  this.$notify({
                    title: "成功",
                    message: "资产归还信息提交成功，请等待管理员审核",
                    type: "success"
                  });
                } else if (val == 1) {
                  this.$notify({
                    title: "成功",
                    message: "资产归还信息成功保存至草稿箱",
                    type: "success"
                  });
                }
              },
          ...
          //console.log(data)
          this.$http
            .post("/assetReturn/reject/submit", data)
            .then(
              res => {
                this.tableData = [];
                this.selectList = [];
